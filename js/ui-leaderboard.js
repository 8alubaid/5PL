...
import { playerAvatarHTML, iconHTML } from './data.js';
...
export function renderLeaderboardTab(){
  ...
  return `<div class="pr-card">
    <div class="pr-section-title">${iconHTML('medal')} ${t('overallStandings')}</div>
    <div style="overflow-x:auto"><table class="pr-table">
      <thead><tr><th></th><th>${t('colPlayer')}</th><th>${t('colTotal')}</th><th>${iconHTML('target')} ${t('colExactBonus')}</th><th>${t('colBestRound')}</th></tr></thead>
      <tbody>${rows}</tbody>
    </table></div>
  </div>`;
}
